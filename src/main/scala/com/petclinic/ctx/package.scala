...
package object ctx {
  ...
  implicit def deriveConcurrentEffectForEnv[F[_]](implicit F: ConcurrentEffect[F]): ConcurrentEffect[Env[F, *]] =
    new ConcurrentEffect[Env[F, *]] {
      ...
      override def start[A](fa: Env[F, A]): Env[F, Fiber[Env[F, *], A]] = concurrent.start(fa)

      override def racePair[A, B](
        fa: Env[F, A],
        fb: Env[F, B]
      ): Env[F, Either[(A, Fiber[Env[F, *], B]), (Fiber[Env[F, *], A], B)]] = concurrent.racePair(fa, fb)

      override def async[A](k: (Either[Throwable, A] => Unit) => Unit): Env[F, A] = concurrent.async(k)
      ...
      override def raiseError[A](e: Throwable): Env[F, A] = concurrent.raiseError(e)

      override def handleErrorWith[A](fa: Env[F, A])(f: Throwable => Env[F, A]): Env[F, A] =
        concurrent.handleErrorWith(fa)(f)

      override def pure[A](x: A): Env[F, A] = concurrent.pure(x)

      override def flatMap[A, B](fa: Env[F, A])(f: A => Env[F, B]): Env[F, B] = concurrent.flatMap(fa)(f)

      override def tailRecM[A, B](a: A)(f: A => Env[F, Either[A, B]]): Env[F, B] = concurrent.tailRecM(a)(f)
    }
